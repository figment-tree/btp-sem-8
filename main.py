...
from model import GCN, GCN_Info, GAT, GAT_Info
...
def main():
    ...
    #  load data
    bundle_train_data, bundle_test_data, assist_data = \
        dataset.get_dataset(
    ...
    #  graph
    ul_graph = bundle_train_data.ground_truth_u_l
    ...
    log = logger.Logger(os.path.join(
        ...
        f"{CONFIG['model']}_{CONFIG['task']}", ''), 'best',
        checkpoint_target=TARGET)

    time_path = time.strftime("%y%m%d-%H%M%S", time.localtime(time.time()))

    for lr, decay, message_dropout, node_dropout \
            in product(CONFIG['lrs'], CONFIG['decays'],
                       CONFIG['message_dropouts'], CONFIG['node_dropouts']):
        ...
        if CONFIG['model'] == 'GAT':
            ...
            info = GAT_Info(64, decay, message_dropout, node_dropout, 2)
            model = GAT(info, assist_data, graph,
                        device, pretrain=None).to(device)
        elif CONFIG['model'] == 'GCN':
            graph = [ul_graph, li_graph]
            info = GCN_Info(64, decay, message_dropout, node_dropout, 2)
            ...
            try:
                ...
                for epoch in range(CONFIG['epochs']):
                    ...
                    if epoch % CONFIG['test_interval'] == 0:
                        ...
                        # check overfitting
                        if epoch > 10:
                            if check_overfitting(log.metrics_log, TARGET, 1,
                                                 show=False):
                                break
                        # early stop
                        early = early_stop(
